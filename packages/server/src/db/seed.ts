...
import { domains, themes, parties, questions } from './schema.js';
...
function parseParties() {
  ...
  const data = yaml.load(raw) as { parties: RawParty[] };
  return data.parties;
}
...
async function main() {
  ...
  console.log(`[seed] Inserting ${rawParties.length} parties...`);
  ...
  console.log(`  - ${rawParties.length} parties`);
